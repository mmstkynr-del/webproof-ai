  ...
  async fetch(request, env) {
    ...
    if (url.pathname === "/api/status" && request.method === "GET") {
      ...
      return json({
        ...
        // Güvenli NVIDIA secret testi.
        // API anahtarının kendisi ASLA gösterilmez.
        secretTest: {
          exists: hasNvidiaKey,
    ...
    if (url.pathname === "/api/tasks" && request.method === "POST") {
      try {
        ...
        if (!normalized) {
          return json({
            ok: false,
            error: "Geçerli HTTP/HTTPS URL gerekli."
          }, 400);
        }
        ...
        const task = {
          ...
          url: normalized,
          command: body.command || "siteyi takip et",
          status: "active",
    ...
    if (url.pathname === "/api/tasks" && request.method === "DELETE") {
      ...
      const index = temporaryTasks.findIndex(x => x.id === id);
...
async function scanWebsite(startUrl, env) {
  ...
  const aiAvailable = Boolean(env.NVIDIA_API_KEY);
  ...
  while (
    ...
    try {
      ...
      if (!response.ok) {
        page.type = "robot-error";
        page.errors.push({
          original: "",
          correction: "",
          reason: `Sayfa HTTP ${response.status} döndürdü.`
        });
      ...
      page.score = scoreArticleUrl(currentUrl);
      ...
      const isArticle =
        page.score >= 40 ||
        (page.score >= 20 && articleStructure);
      ...
      if (isArticle) {
        ...
        page.errors = ruleAnalysis.errors;
        page.suspicious = ruleAnalysis.suspicious;

        if (aiAvailable && aiCount < MAX_ARTICLES_FOR_AI) {
          try {
            const aiResult =
            ...
            if (aiResult.ok) {
              page.aiStatus = "connected";
              page.aiErrors = aiResult.errors || [];
              aiCount++;
            } else {
              page.aiStatus = "error";
              page.aiErrors = [{
                ...
                type: "diğer",
                confidence: 0,
                reason: aiResult.error || "NVIDIA AI hatası."
              }];
...
function extractLinks(html, hostname) {
  ...
  while ((match = regex.exec(html)) !== null) {
    ...
    try {
      const url = new URL(raw, `https://${hostname}`);
...
function scoreArticleUrl(targetUrl) {
  try {
...
function ruleBasedProofread(text) {
  ...
  while (
    (match = beforePunctuation.exec(text)) !== null
  ) {
    const original =
      match[1] + match[0].slice(1);
  ...
  while (
    (match = duplicatePunctuation.exec(text)) !== null
  ) {
  ...
  while (
    (match = missingSpace.exec(text)) !== null
  ) {
    ...
    // URL, domain ve e-mail yapılarında raporlama yapma
    if (
      /https?:\/\/[^\s]*$/i.test(before) ||
      /www\.[^\s]*$/i.test(before) ||
      /[\w.+-]+@[\w.-]+\.[A-Za-z]{2,}$/i.test(before)
    ) {
      continue;
    }
  ...
  const replacements = [
    [
      ...
      "birçok",
      "Yazım",
      "Bu kelime Türkçede genellikle bitişik yazılır."
    ],
    [
      /\bhiç bir\b/gi,
      "hiçbir",
    ...
    [
      ...
      "yanlış",
      "Yazım",
      "Kelimenin doğru yazımı 'yanlış' şeklindedir."
    ],
    [
      /\byanlız\b/gi,
      "yalnız",
      "Yazım",
      "Kelimenin doğru yazımı 'yalnız' şeklindedir."
    ],
    [
      /\bherkez\b/gi,
      "herkes",
      "Yazım",
      "Kelimenin doğru yazımı 'herkes' şeklindedir."
    ],
    [
      /\bbirşey\b/gi,
      "bir şey",
...
Yalnızca gerçekten hata olduğundan yüksek derecede emin olduğun
yazım, noktalama, dilbilgisi veya sayı kullanım hatalarını bildir.

Gazetecilik üslubunu değiştirme.
Cümleleri gereksiz yere yeniden yazma.
Doğru olan kullanımları hata olarak bildirme.
Özel isimleri değiştirme.
Alıntıları gereksiz yere değiştirme.

Yanıtı SADECE aşağıdaki JSON yapısında ver:
...
confidence değeri 0 ile 1 arasında olsun.

Sadece confidence >= 0.85 olan gerçek hataları döndür.
Hata yoksa:
{
  "errors": []
}
  ...
  try {
    ...
    try {
      ...
      return {
        ok: false,
        error:
          "NVIDIA model yanıtı ayrıştırılamadı."
      };
    }
  }
...
function safeErrorMessage(error) {
  const message =
    String(error?.message || error || "Bilinmeyen hata");

  // API anahtarının hata mesajına yanlışlıkla sızmasını engelle
  return message
