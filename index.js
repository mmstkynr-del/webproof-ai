# ============================================================
# WEBPROOF AI — CLOUDFLARE AI CORE
# Gemini + NVIDIA
# ============================================================

# ------------------------------------------------------------
# 1) GEMINI API KEY'İ CLOUDFLARE SECRET OLARAK EKLE
# ------------------------------------------------------------
#
# Önce Google AI Studio'dan Gemini API key oluştur.
#
# Sonra Cloudflare Worker projenin bulunduğu ortamda:
#
# npx wrangler secret put GEMINI_API_KEY
#
# Komut senden anahtarı isteyecek.
# Anahtarı buraya yapıştır.
#
# NOT:
# API anahtarını bu dosyaya yazma.
# GitHub'a yazma.
# Frontend JavaScript'e yazma.
#
# NVIDIA_API_KEY zaten Cloudflare Secret olarak varsa
...
async function fetchWithTimeout(url, options = {}, timeout = FETCH_TIMEOUT) {
  ...
  try {
    return await fetch(url, {
      ...
      headers: {
        "user-agent":
          "WebProofAI/2.0 (+https://webproof.ai)",
        "accept":
...
function articleScore(url, rawHtml, text) {
  ...
  if (/<article\b/i.test(rawHtml)) score += 30;
  ...
  if (
    /og:type["']?\s*content=["']article/i.test(lower)
  ) {
    score += 20;
  }

  if (text.length >= 1000) score += 10;
  if (text.length >= 2500) score += 10;
...
function detectLanguage(text) {
  const turkishChars =
    (text.match(/[çğıöşüÇĞİÖŞÜ]/g) || []).length;
...
// ============================================================
// DETERMINISTIC — SADECE OBJEKTİF YAZIM HATALARI
// ============================================================
...
function deterministicErrors(text, language) {
  ...
  for (const [wrong, correct] of Object.entries(rules)) {
    ...
    while ((match = regex.exec(text)) !== null) {
      errors.push({
        original: match[2],
        correction: preserveCase(match[2], correct),
        type: "spelling",
        confidence: 1,
        reason: "Objektif yazım hatası."
      });
    }
  }
...
function preserveCase(original, correction) {
  ...
  if (
    ...
    return correction.charAt(0).toUpperCase() +
      correction.slice(1);
  }
...
Your job is NOT to rewrite text.

Find ONLY objectively verifiable:
- spelling errors
- grammar errors
- punctuation errors
...
- dialect differences
- capitalization preferences unless objectively wrong
- names
- surnames
- organizations
- brands
...
unless there is clear objective evidence of an error.

CRITICAL RULES:

1. "original" MUST be an exact substring from the supplied text.
2. "correction" must be the smallest possible correction.
3. Never rewrite an entire sentence.
4. Never invent an error.
5. Confidence must be >= 0.90.
...
function splitIntoChunks(text, size = AI_CHUNK_SIZE) {
  if (text.length <= size) {
    return [text];
  }
...
function validateAIResult(result, sourceText) {
  ...
  for (const e of result.errors) {
    ...
    if (confidence < MIN_AI_CONFIDENCE) continue;
...
async function callNvidia(env, text, language) {
  ...
  const body = {
    ...
    // --------------------------------------------------------
    // ÇOK ÖNEMLİ:
    // extra_body KESİNLİKLE YOK.
    //
    // Önceki 400 hatasının sebebi buydu:
...
async function analyzeWithAI(
  ...
  if (env.GEMINI_API_KEY) {
    ...
    // Gemini çalışıyorsa ana motor olarak kullan.
    if (geminiFailures === 0) {
      return {
        provider: "gemini",
        errors: dedupeErrors(
...
<div class="sub">
Gerçek web taraması + editoryal doğrulama +
...
async function scan() {
  ...
  if (!url) {
    alert("Bir web sitesi adresi gir.");
    return;
  }

  document.getElementById("status").innerHTML =
    "<div class='card'>Tarama yapılıyor...</div>";
  ...
  try {
    ...
    document.getElementById("results").innerHTML =
      ...
      data.pages +
      "</b><br>" +
      "<span class='small'>Taranan sayfa</span>" +
      "</div>" +
      ...
      "</b><br>" +
      "<span class='small'>Bulunan haber</span>" +
      "</div>" +
      ...
      data.totalErrors +
      "</b><br>" +
      "<span class='small'>Toplam hata</span>" +
      "</div>" +
      ...
      data.articles.map(article => {
        ...
        if (!article.errors.length) {
          ...
          body =
            ...
            "objektif yazım/dilbilgisi hatası " +
            "bulunmadı." +
            "</div>";
